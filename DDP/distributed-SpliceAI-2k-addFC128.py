...
#from dataloaderBig_seed_250329 import train_loader, val_loader, test_loader
...
class SpliceAI2kModel(nn.Module):
    def __init__(self, input_channels=4, num_classes=2, hidden_dim=128): #改为128宽度，尝试
...
model = SpliceAI2kModel(input_channels=4, num_classes=2, hidden_dim=64)
...
for epoch in range(num_epochs):
    ...
    if best_f1_score is None or avg_f1_score > best_f1_score:
        ...
        counter = 0
        torch.save(model.state_dict(), 'best_CNNRB_FC128_model.pth')
...
# 测试
model.load_state_dict(torch.load('best_CNNRB_FC128_model.pth'))
model.eval()
test_loss = 0
f1_metric.reset()
conf_matrix_metric.reset()
