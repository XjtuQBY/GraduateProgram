...
import h5py
import numpy as np
import torch.nn.functional as F  # 导入F模块用于Softmax操作
...
class SpliceAI2kCombinedModel(nn.Module):
    def __init__(self, input_channels=4, num_classes=2, hidden_dim=64):
        super(SpliceAI2kCombinedModel, self).__init__()
        ...
        self.fc = nn.Sequential(
            nn.Conv1d(32, hidden_dim, kernel_size=1),  # 使用1x1卷积代替全连接，保持序列结构
            nn.BatchNorm1d(hidden_dim),
            nn.ReLU()
        )

        # 最后卷积层
        self.last_conv = nn.Conv1d(hidden_dim, num_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # 改进1：应用可学习的权重进行特征融合
        # 使用softmax确保权重归一化，让模型更容易学习相对重要性
        norm_weights = F.softmax(self.skip_weights, dim=0)

        # 加权融合所有特征
        result = norm_weights[0] * x + norm_weights[1] * skip1 + norm_weights[2] * skip2 + norm_weights[3] * skip3
...
# 使用结合了两种改进的模型
model = SpliceAI2kCombinedModel(input_channels=4, num_classes=2, hidden_dim=64)
...
test_accuracy_metric = torchmetrics.Accuracy(num_classes=output_size, task="binary").to(device)
...
for epoch in range(num_epochs):
    ...
    val_accuracy_metric.reset()  # 重置验证准确率指标
    ...
    with torch.no_grad():
        for inputs, labels in val_loader:
            ...
            val_accuracy_metric.update(val_preds, labels)  # 更新验证准确率

    avg_val_loss = val_loss / len(val_loader)
    val_accuracy = val_accuracy_metric.compute()  # 计算验证准确率
    ...
    print(
        ...
        f'Val Loss: {avg_val_loss:.4f}, Val Accuracy: {val_accuracy:.4f}, F1 Score: {f1_score}, '
    ...
    # 打印当前的跳跃连接权重，用于监控
    skip_weights = F.softmax(model.module.skip_weights, dim=0).cpu().detach().numpy()
    print(f'Current skip connection weights: {skip_weights}')
    ...
    if best_f1_score is None or avg_f1_score > best_f1_score:
        ...
        counter = 0
        torch.save(model.state_dict(), 'best_Combined_FC_SkipWeight_model.pth')
...
# 测试
model.load_state_dict(torch.load('best_Combined_FC_SkipWeight_model.pth'))
model.eval()
test_loss = 0
f1_metric.reset()
conf_matrix_metric.reset()
test_accuracy_metric.reset()  # 重置准确率指标
...
with torch.no_grad():
    for inputs, labels in test_loader:
        ...
        test_accuracy_metric.update(test_preds, labels)  # 更新准确率指标
...
# 输出最终学习到的权重
final_weights = F.softmax(model.module.skip_weights, dim=0).cpu().detach().numpy()
print(f'Final learned skip connection weights: {final_weights}')