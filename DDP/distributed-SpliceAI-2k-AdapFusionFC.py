...
# Uncomment the following line if you want to use the big dataloader instead
...
if __name__ == "__main__":
    ...
    accuracy_metric = torchmetrics.Accuracy(num_classes=output_size, task="binary").to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if best_f1_score is None or avg_f1_score > best_f1_score:
            ...
            counter = 0
            torch.save(model.state_dict(), 'best_Combined_FC_AdapFusion_model.pth')
    ...
    # 测试
    model.load_state_dict(torch.load('best_Combined_FC_AdapFusion_model.pth'))
    model.eval()
    test_loss = 0
    f1_metric.reset()
    conf_matrix_metric.reset()
