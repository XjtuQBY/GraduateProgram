...
import h5py
import numpy as np
import torch.nn.functional as F  # 新增导入F模块
...
class SpliceAI2kModelImproved(nn.Module):
    def __init__(self, input_channels=4, num_classes=1):
        super(SpliceAI2kModelImproved, self).__init__()
    ...
    def forward(self, x):
        ...
        # norm_weights = F.softmax(self.skip_weights, dim=0) * 4  # 乘以4使总和为4
...
# 使用改进的模型
model = SpliceAI2kModelImproved(input_channels=4, num_classes=2)
...
for epoch in range(num_epochs):
    ...
    val_f1_score = f1_metric.compute()
    ...
    print(
        ...
        f'Val Loss: {avg_val_loss:.4f}, Val Accuracy: {val_accuracy:.4f}, F1 Score: {val_f1_score}, '
    ...
    # 早停机制
    avg_f1_score = val_f1_score.mean().item()
    if best_f1_score is None or avg_f1_score > best_f1_score:
        best_f1_score = avg_f1_score
        counter = 0
        torch.save(model.state_dict(), 'best_SkipWeight_model.pth')
...
# 测试
model.load_state_dict(torch.load('best_SkipWeight_model.pth'))
model.eval()
test_loss = 0
f1_metric.reset()
conf_matrix_metric.reset()
