...
def find_optimal_threshold(model, data_loader, device):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(data_loader, desc="Finding optimal threshold"):
    ...
    for threshold in tqdm(thresholds, desc="Testing thresholds"):
        ...
        # 计算敏感性(Sensitivity/Recall)和特异性(Specificity)
        ...
        # 计算敏感性和特异性之间的差异的绝对值
        difference = abs(sensitivity - specificity)
    ...
    # 输出最佳阈值及相关指标
    print(f"最佳平衡阈值: {best_threshold:.3f}")
    print(
        f"在此阈值下 - 敏感性(Sn): {best_sn:.4f}, 特异性(Sp): {best_sp:.4f}, 差异: {min_difference:.4f}, F1: {best_f1:.4f}")

    # 可选：绘制敏感性和特异性随阈值变化的曲线
    try:
        ...
        # 第一个子图：敏感性和特异性
        plt.subplot(2, 1, 1)
        plt.plot(thresholds, sensitivities, 'b-', label='敏感性(Sn)')
        plt.plot(thresholds, specificities, 'r-', label='特异性(Sp)')
        plt.plot(thresholds, differences, 'g--', label='|Sn-Sp|')
        plt.axvline(x=best_threshold, color='k', linestyle='--',
                    label=f'最佳阈值={best_threshold:.3f}')
        plt.xlabel('阈值')
        plt.ylabel('性能')
        plt.title('敏感性(Sn)和特异性(Sp)随阈值变化')
        plt.legend()
        plt.grid(True)

        # 第二个子图：F1分数
        plt.subplot(2, 1, 2)
        plt.plot(thresholds, f1_scores, 'b-', label='F1分数')
        plt.axvline(x=best_threshold, color='k', linestyle='--',
                    label=f'最佳阈值={best_threshold:.3f}')
        plt.xlabel('阈值')
        plt.ylabel('F1分数')
        ...
        print("阈值优化曲线已保存为 threshold_optimization.png")
    except Exception as e:
        print(f"无法绘制曲线图：{str(e)}")

    return best_threshold, best_f1, results


# 修改后的训练函数
# 训练函数 - 不寻找最佳阈值，始终使用0.5
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100, device='cuda',
    ...
    val_f1s = []
    val_aurocs = []
    best_val_auroc = 0  # 使用AUROC进行早停
    epochs_without_improvement = 0
    threshold = 0.5  # 始终使用默认阈值0.5
    ...
    for epoch in range(num_epochs):
        ...
        # 使用默认阈值0.5计算所有指标
        preds_binary = (all_probs_tensor > threshold).int()
        val_f1 = f1_metric(preds_binary, all_labels_int)
        val_auroc = auroc_metric(all_probs_tensor, all_labels_int)
        val_precision = precision_metric(preds_binary, all_labels_int)
        val_recall = recall_metric(preds_binary, all_labels_int)
        val_conf_matrix = conf_matrix_metric(preds_binary, all_labels_int)

        # 保存指标
        val_f1s.append(val_f1.item())
        val_aurocs.append(val_auroc.item())
        ...
        print(f"Validation Confusion Matrix (threshold={threshold:.3f}):\n{val_conf_matrix}")

        # 更新学习率
        if isinstance(scheduler, optim.lr_scheduler.ReduceLROnPlateau):
            scheduler.step(val_auroc)  # 使用AUROC来调整学习率
        ...
        if val_auroc > best_val_auroc:
            ...
            torch.save({
                ...
                'val_f1': val_f1.item()
            }, 'DNABERT_Hum_FinalTheashold.pth')
            print(f"验证AUROC提高到 {best_val_auroc:.4f}。模型已保存。")
        else:
            epochs_without_improvement += 1
            print(f"AUROC已连续 {epochs_without_improvement} 个epoch没有提高。当前最佳AUROC: {best_val_auroc:.4f}")
...
# 评估函数 - 首先找到最佳阈值，然后用该阈值进行评估
def evaluate_model(model, test_loader, criterion, device='cuda'):
    model.eval()
    test_loss = 0.0

    print("第1阶段: 寻找最佳阈值...")
    best_threshold, _, threshold_results = find_optimal_threshold(model, test_loader, device)

    print(f"\n第2阶段: 使用阈值 {best_threshold:.3f} 进行最终评估...")
    ...
    # 使用原始概率输出计算AUROC（这个指标不依赖于阈值）
    auroc = auroc_metric(all_probs_tensor, all_labels_int)

    # 使用最佳阈值的二值化预测计算其他指标
    all_preds_binary = (all_probs_tensor > best_threshold).int()

    # 计算准确率
    correct = (all_preds_binary == all_labels_int).sum().item()
    total = all_labels_int.size(0)
    ...
    print(f"敏感性(Sensitivity/Recall): {recall:.4f}")
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load('DNABERT_Hum_FinalTheashold.pth')
    model.load_state_dict(checkpoint['model_state_dict'])
    print(f"Loaded model from epoch {checkpoint['epoch']} with validation AUROC: {checkpoint['val_auroc']:.4f}")
    ...
    with open('test_metrics.json', 'w') as f:
        # 将threshold_results从保存的指标中移除，因为它可能很大
        metrics_to_save = {k: v for k, v in test_metrics.items() if k != 'threshold_results'}
        # 将numpy数组转换为列表
        metrics_json = {k: v.tolist() if isinstance(v, np.ndarray) else v for k, v in metrics_to_save.items()}
        json.dump(metrics_json, f, indent=4)

    # 单独保存阈值优化结果
    import pandas as pd

    if 'threshold_results' in test_metrics and test_metrics['threshold_results']:
        threshold_df = pd.DataFrame(test_metrics['threshold_results'])
        threshold_df.to_csv('threshold_optimization_results.csv', index=False)
        print("阈值优化详细结果已保存至 threshold_optimization_results.csv")
