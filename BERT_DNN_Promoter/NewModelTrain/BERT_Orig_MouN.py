...
def find_optimal_threshold(model, val_loader, device):
    ...
    print(f"在此阈值下 - 敏感性(Sn): {best_sn:.4f}, 特异性(Sp): {best_sp:.4f}, 差异: {min_difference:.4f}, F1: {best_f1:.4f}")
    ...
    try:
        ...
        plt.axvline(x=best_threshold, color='k', linestyle='--', label=f'最佳阈值={best_threshold:.3f}')
        plt.xlabel('阈值')
        plt.ylabel('性能')
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100, device='cuda',
    ...
    val_f1s = []
    val_aurocs = []
    best_val_auroc = 0  # 使用AUROC分数进行早停
    ...
    for epoch in range(num_epochs):
        ...
        if val_auroc > best_val_auroc:
            ...
            torch.save({
                ...
                'val_f1': val_f1.item()
            }, 'Enhanced_BERT_Promoter_model_MouNonTATA.pth')
            print(f"验证AUROC提高到 {best_val_auroc:.4f}。模型已保存。最佳阈值: {best_threshold:.3f}，F1: {val_f1:.4f}")
        else:
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load('Enhanced_BERT_Promoter_model_MouNonTATA.pth')
