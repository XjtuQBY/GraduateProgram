...
# 修改后的BERTSpliceModel，添加可学习的跳跃连接权重
class BERTSpliceModelImproved(nn.Module):
    def __init__(self, input_channels=768, num_classes=1):
        super(BERTSpliceModelImproved, self).__init__()
        ...
        for i in range(3):
            self.rb_group2.append(ResidualBlock(channels=32, kernel_size=11, dilation=2))
        ...
        for i in range(3):
            self.rb_group3.append(ResidualBlock(channels=32, kernel_size=21, dilation=2))
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100, device='cuda',
    ...
    for epoch in range(num_epochs):
        ...
        # 打印当前的跳跃连接权重，用于监控
        skip_weights = F.softmax(model.skip_weights, dim=0).cpu().detach().numpy()
        ...
        if val_loss < best_val_loss:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), 'DBP_SpliceAI_Arab_TATA_improved.pth')
...
# 修改评估函数，确保precision和recall指标计算正确
def evaluate_model(model, test_loader, criterion, device='cuda'):
    ...
    # 打印最终的跳跃连接权重
    skip_weights = F.softmax(model.skip_weights, dim=0).cpu().detach().numpy()
    print(f"Final learned skip connection weights: {skip_weights}")
...
if __name__ == "__main__":
    ...
    # 创建改进的模型实例
    model = BERTSpliceModelImproved().to(device)  # 将模型移动到指定设备（GPU/CPU）
    ...
    model.load_state_dict(torch.load('DBP_SpliceAI_Arab_TATA_improved.pth'))
    ...
    # 保存测试指标到文件
    with open('test_metrics_improved.json', 'w') as f:
    ...
    model_scripted.save('bert_splice_model_improved.pt')  # 保存为TorchScript模型
