...
def extract_bert_features_numpy(input_pattern, output_prefix, bert_model_path, max_seq_length=510, k=6,
                                do_lower_case=False, label=None, save_csv=True, exclude_special_tokens=True):
    """
    从输入序列提取BERT特征并保存为多种格式，只提取最后一层

    参数:
    input_pattern: 输入文件模式，支持通配符
    output_prefix: 输出文件前缀（会添加后缀.npz,.csv等）
    bert_model_path: BERT模型路径
    max_seq_length: 最大序列长度
    k: k-mer大小
    do_lower_case: 是否将文本转为小写
    label: 数据标签（0表示非启动子，1表示启动子）
    save_csv: 是否同时保存CSV格式（便于检查）
...
def process_promoter_data_numpy(promoter_input, non_promoter_input, output_dir, bert_model_path, max_seq_length=510,
                                k=6, save_csv=True, exclude_special_tokens=True):
    """
...
def main():
    ...
    print(f"工作目录: {os.path.dirname(full_path)}")
    ...
    datasets = ["C_JEJUNI_1", "C_JEJUNI_2", "C_JEJUNI_3", "C_JEJUNI_4", "C_JEJUNI_5",
                ...
                "BACILLUS", "CLOSTRIDIUM", "MYCOBACTER", "RHODOBACTER_1", "RHODOBACTER_2"]

    # k-mer大小
    k = 6

    # 设置序列长度和对应的token数量
    sequence_length = 40  # 你的实际序列长度，例如40bp
    ...
    elapsed_time = time.time() - start_time
    print("\n" + "=" * 50)
    print("所有数据集的BERT特征提取完成!")
    print(f"总用时: {elapsed_time:.2f} 秒")
    print("=" * 50)

    print(f"\n请使用这些中间文件在process2h5环境中运行npz2h5.py来生成最终的H5文件")
