...
class BERTSpliceModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # 最终卷积层 - 保持通道数仍为base_channels
        x = self.final_conv(x)

        # 全局平均池化，将序列长度维度池化，
        # 输入: [batch_size, base_channels, sequence_length]
        # 输出: [batch_size, base_channels]
        #x = torch.mean(x, dim=2)
        x = torch.max(x, dim=2)[0]  # [batch_size, base_channels]
        """未来还可以考虑——————全局最大池化、注意力池化
        # 全局最大池化 - 关注最显著的特征
        x = torch.max(x, dim=2)[0]  # [batch_size, base_channels]
        # 注意力池化 - 学习不同位置的重要性
        attention = nn.Conv1d(base_channels, 1, kernel_size=1)(x)
        attention = F.softmax(attention, dim=2)
        x = torch.sum(x * attention, dim=2)  # [batch_size, base_channels]"""
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100, device='cuda',
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), 'best_BERT_SpliceAI_Channel64LR001AddFC_model.pth')
...
if __name__ == "__main__":
    ...
    criterion = FocalLoss(alpha=0.65, gamma=3.0)  # 使用你指定的 alpha=0.66 和 gamma=2
    ...
    model.load_state_dict(torch.load('best_BERT_SpliceAI_Channel64LR001AddFC_model.pth'))
