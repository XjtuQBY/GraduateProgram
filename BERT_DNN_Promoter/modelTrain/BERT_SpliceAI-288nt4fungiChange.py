...
class BERTSpliceModel(nn.Module):
    def __init__(self, input_channels=768, num_classes=1):
        ...
        # 最终卷积层
        self.final_conv = nn.Conv1d(base_channels, num_classes, kernel_size=1)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100, device='cuda',
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1:
            ...
            print(f"Validation F1 improved to {best_val_f1:.4f}. Model saved.")
        else:
            epochs_without_improvement += 1
            print(f"No improvement in F1 for {epochs_without_improvement} epochs. Best F1: {best_val_f1:.4f}")
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('best_BERT_SpliceAI_Channel4_model.pth'))
