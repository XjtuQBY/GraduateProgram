...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100, device='cuda',
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), 'best_BERT_SpliceAI_Channel_model.pth')
...
if __name__ == "__main__":
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.01, weight_decay=1e-5)  # 使用Adam优化器，带L2正则化
    ...
    model.load_state_dict(torch.load('best_BERT_SpliceAI_Channel_model.pth'))
