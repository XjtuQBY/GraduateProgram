...
class CNNRBModel(nn.Module):
    def __init__(self, input_channels=4, num_classes=2):
        ...
        self.rb_group1 = nn.ModuleList([
            ResidualBlock(channels=64, kernel_size=11, dilation=1)
            for _ in range(6)
        ])
        ...
        self.rb_group2 = nn.ModuleList([
            ResidualBlock(channels=64, kernel_size=11, dilation=4)
            for _ in range(6)
        ])
        ...
        # 第三组残差块：6个RB(64, 21, 10)
        self.rb_group3 = nn.ModuleList([
            ResidualBlock(channels=64, kernel_size=21, dilation=10)
            for _ in range(6)
        ])

        # 第三个中间卷积层
        self.middle_conv3 = nn.Sequential(
            nn.Conv1d(64, 64, kernel_size=3, padding=1),
            nn.BatchNorm1d(64),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        # 中间卷积 + 跳跃连接
        x = self.middle_conv1(x) + skip_connections[0]
...
learning_rate = 0.0005  # 学习率在使用余弦退火时可以设置更高一点
output_size = 2 #二分类问题
num_epochs = 25
...
for epoch in range(num_epochs):
    ...
    if best_f1_score is None or avg_f1_score > best_f1_score:
        ...
        counter = 0
        torch.save(model.state_dict(), 'best_CNNRBer_model1.0.pth')
...
'''
# 测试
model.load_state_dict(torch.load('best_CNNRBer_model1.0.pth'))
model.eval()
test_loss = 0
f1_metric.reset()
conf_matrix_metric.reset()
